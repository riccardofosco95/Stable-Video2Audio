...
from stable_audio_tools.models.adp import NumberEmbedder
...
class CLAPAudioConditioner(Conditioner):
    ...
    def forward(self, audios: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]] , device: tp.Any = "cuda") -> tp.Any:
        ...
        # print(f"Audio embedding CLAP:", audio_embedding.shape)

        return [self.proj_out(audio_embedding), torch.ones(audio_embedding.shape[0], 1).to(device)]
    
class CAVPConditioner(Conditioner):
    def __init__(self, 
                 output_dim: int, 
                 config_path=None, 
                 ckpt_path=None, 
        ...
        self.stage1_model = self.instantiate_from_config(config.model)
    ...
    @torch.no_grad()
    def forward(self, x: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]], device: tp.Any = "cuda"):
        self.stage1_model.to(device)

        if isinstance(x, tuple):
            frames, spec = x
            audio_contrastive_feats = self.stage1_model.encode_spec(spec.to(device), normalize=True, pool=False)
        else:
            frames = x
        ...
        frames = frames.permute(0, 2, 1, 3, 4).to(device) # Video: B x 3 x T x H x W -> B x T x 3 x H x W   
        video_contrastive_feats = self.stage1_model.encode_video(frames, normalize=True, pool=False)
        video_contrastive_feats = video_contrastive_feats.unsqueeze(1).to(device)
        if audio_contrastive_feats is not None:
            audio_contrastive_feats = audio_contrastive_feats.unsqueeze(1).to(device)
...
class PretransformConditioner(Conditioner):
    ...
    def forward(self, audio: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]], device: tp.Union[torch.device, str]) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        ...
        if isinstance(audio, list) or isinstance(audio, tuple):
            # print(f"Audio dimension: {audio[0].shape}")
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any]) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "cavp_frames":
            conditioners[id] = CAVPConditioner(**conditioner_config)
